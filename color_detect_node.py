...
class ColorDetectNode(Node):
    ...
    def main(self):
        while self.running:
            ...
            h, w = image.shape[:2]
            img_lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)  # bgr转lab(convert bgr to lab)
            ...
            for i in self.target_colors:
                if self.detect_type[i] == 'line' and i != '':
                    ...
                    for roi in self.line_roi:
                        ...
                        contours = cv2.findContours(dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)[-2]  # 找轮廓(find contour)
                        max_contour_area = common.get_area_max_contour(contours, 400)[0]  # 获取最大面积对应轮廓(obtain the contour corresponding to the maximum area)
                        if max_contour_area is not None:
                            rect = cv2.minAreaRect(max_contour_area)  # 最小外接矩形(the minimum bounding rectangle)
                            box = np.intp(cv2.boxPoints(rect))  # 四个角(four corners)
                            for j in range(4):
                                box[j, 1] = box[j, 1] + roi_value[0]
                            cv2.drawContours(result_image, [box], -1, common.range_rgb[i], 2)  # 画出四个点组成的矩形(draw a rectangle formed by four points)
                            ...
                            cv2.circle(result_image, (int(line_center_x), int(line_center_y)), 5, common.range_rgb[i], -1)  # 画出中心点(draw center point)
                    ...
                    if max_contour_area is not None:
                        ...
                        if self.detect_type[i] == 'rect':
                            if area > max_area_rect:  # 找最大面积(find the maximal area)
                                max_area_rect = area
                                color_area_max_rect = i
                                areaMaxContour_rect = max_contour_area
                        else:
                            if area > max_area_circle:  # 找最大面积(find the maximal area)
                                max_area_circle = area
                                color_area_max_circle = i
                                areaMaxContour_circle = max_contour_area
            colors_info = ColorsInfo()
            color_info_list = []
            center_pos = centroid_sum / self.weight_sum  # 按比重计算中心点(calculate the center point by proportional weights)
            ...
            if areaMaxContour_rect is not None:
                ...
                cv2.drawContours(result_image, [box], -1, common.range_rgb[color_area_max_rect], 2)  # 画出四个点组成的矩形(draw the rectangle formed by four points)
            ...
            if areaMaxContour_circle is not None:
                ((x, y), radius) = cv2.minEnclosingCircle(areaMaxContour_circle)  # 获取最小外接圆(get the minimum bounding circumcircle)
